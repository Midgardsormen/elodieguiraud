...
angular.module( 'jackmail.directives' ).directive( 'jackmailHeaderMenu', [
	function() {
		return {
			...
			controllerAs: 'djhm',
			template: [
				'<div class="jackmail_header_button_menu jackmail_dropdown_button_container jackmail_dropdown_button_left_container"' +
				'	  ng-mouseleave="djhm.hide_menu_button()" ng-click="djhm.display_or_hide_menu_button()">' +
				'	<span class="jackmail_header_logo"></span>' +
				'	<span class="jackmail_header_logo_text">Jackmail</span>' +
				'	<span class="jackmail_header_dropdown_button"></span>' +
...
angular.module( 'jackmail.directives' ).directive( 'ngSrcLoaded', [
	function() {
		return {
			restrict: 'A',
			link: function(scope, element, attrs) {
				element.bind('load', function() {
					scope.$apply(attrs.ngSrcLoaded)(true);
				});
				element.bind('error', function(){
					scope.$apply(attrs.ngSrcLoaded)(false);
				});
			}
		};
	} ] );
	...
	function( $timeout, $rootScope, $http ) {
		return {
			...
			template: [
				...
				'		<span class="dashicons dashicons-search"></span>' +
				'		<span class="jackmail_header_search_text">{{$root.translations.needHelp}}</span>' +
				'	</span>' +
				'	<span ng-click="djs.display_apps()" class="jackmail_header_apps_icon"></span>' +
			...
			controller: [ function() {
				...
				vm.display_apps = function() {
					var lang = $rootScope.settings.language;
					var url = 'https://www.sarbacanegroup-cdn.com/products_modal/?lang=' + lang + '&selectedProduct=jackmail&selectedProductConnected=true';
					$http.get( url ).then( function( args ) {
						angular.element( 'body' ).append( args.data );
					});
				};
