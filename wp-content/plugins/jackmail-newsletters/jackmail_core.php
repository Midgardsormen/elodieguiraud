...
class Jackmail_Core {
	...
	public function update_jackmail_database() {
		...
		$sql = "ALTER TABLE `{$wpdb->prefix}jackmail_scenarios_events` CHANGE `status_detail` `status_detail` varchar(50) NOT NULL DEFAULT ''";
		$wpdb->query( $sql );
		$sql = "ALTER TABLE `{$wpdb->prefix}jackmail_campaigns` CHANGE `status_detail` `status_detail` varchar(50) NOT NULL DEFAULT ''";
		$wpdb->query( $sql );
	...
	private function generate_content_email_preview( $preview, $content_email_json, $content_email_html, $content_email_txt ) {
		...
		if ( $content_email !== '' ) {
			...
			if ( '' !== $content_email_json ) {
				$body['wizardContent'] = json_decode( $content_email );
			} else if ( '' !== $content_email_html ) {
				$body['htmlContent'] = $content_email;
			} else {
				$body['textContent'] = $content_email;
			}
			$headers  = array(
				'content-type' => 'application/json'
			);
					...
					if ( isset( $response['response']['message'] ) ) {
						if ( $response['response']['message'] === 'OK' ) {
							if ( isset( $response['body'] ) ) {
								...
								if ( strpos( $content_email_png_file, 'PNG' ) !== -1 ) {
	...
	public function get_jackmail_url_thumbnail() {
		return $this->get_jackmail_url_global() . 'v2/thumbnail';
	}
	...
	public function get_jackmail_url_ws2() {
		return 'https://gallery.jackmail.com/';
	}
								...
								foreach ( $credits_array as $credit_array ) {
									if ( isset( $credit_array['creditType'], $credit_array['count'],
										...
										if ( strtotime( $current_date_gmt ) - strtotime( $expiration_date_gmt ) > 0 ) {
											...
											$days_until_expiration = intval( - ( strtotime( $current_date_gmt ) - strtotime( $expiration_date_gmt ) ) / 86400 );
										}
										$product_key = '';
	...
	public function get_gallery_template_json( $gallery_id, $link_tracking ) {
		if ( $this->emailbuilder_installed() ) {
			$url      = $this->get_jackmail_url_ws2() . 'gallery.php?product=jackmail&id=' . $gallery_id;
			$headers  = array();
			$timeout  = 30;
